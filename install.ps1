...
Under `irm … | iex` this runs in the caller's session: the script never exits that session, and it throws on a non-zero `bs onboard`.
#>
...
try {
  ...
  # Under `irm … | iex` this runs in the caller's session, so never `exit` here — it would close their shell.
  if ($LASTEXITCODE -ne 0) { throw "bs onboard exited $LASTEXITCODE" }
} finally {
