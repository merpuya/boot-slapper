...
- The *applied* config-library entry is what Desktop and Cowork read, so `desktop-mcp` and `desktop-skills` read it (`appliedEntry`) whether or not it is ours; `ourEntry` is only for writing. When `desktop-inference` adopts a foreign applied gateway entry, `desktop-mcp` is `blocked` with an in-app instruction, never writes servers anywhere, and `desktop-mcp` needs the same `baseUrl` option as `desktop-inference` to know that.
