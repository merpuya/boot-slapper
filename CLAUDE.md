...
- Adopt path dead-ends the other two desktop artifacts. When `desktop-inference` adopts a *foreign* applied entry (it already satisfies the profile, so it is `present` and no boot-slapper entry is ever written), `desktop-mcp` reports "no boot-slapper entry yet — desktop-inference creates it earlier in this run" on every run and never gets one, and `desktop-skills` falls back to `ORG_SENTINEL` for the org segment even though the applied entry may set `deploymentOrganizationUuid`. Intended fix: a distinct blocked reason on `desktop-mcp` — the applied configuration is not boot-slapper's and is never edited, so add the servers in the app, or rename/remove that entry and re-run — and `desktop-skills` should read `deploymentOrganizationUuid` from the *applied* entry, foreign or not, rather than only from ours.
