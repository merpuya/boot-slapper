...
- The *applied* config-library entry is what Desktop and Cowork read, so `desktop-mcp` and `desktop-skills` read it (`appliedEntry`) whether or not it is ours; `ourEntry` is only for writing. When `desktop-inference` adopts a foreign applied gateway entry, `desktop-mcp` is `blocked` with an in-app instruction, never writes servers anywhere, and `desktop-mcp` needs the same `baseUrl` option as `desktop-inference` to know that. Desktop re-serializes the applied entry on launch and after an OAuth Connect (default keys added, `oauth: true` → `{ mode: "dcr" }`), so `desktop-mcp` compares servers with `sameServer` (oauth by presence) and keeps the app's oauth object when it rewrites.
...
- Desktop, verified on JCB-AL-ACA34 2026-09-16 via the adopt route (v1 flat applied entry): managed servers written into the library load — `~/Library/Logs/Claude-3p/main.log` shows mecp connected through the headers helper (97 tools) and openbrain via OAuth after Connect; the `open-brain-auth` Desktop grant check turns ✓ after the first Connect; Desktop rewrites the applied entry on launch (adds `autoModeEnabled`, `chatTabEnabled`, `claudeAiImport`, `inferenceCredentialKind`, `isDesktopExtensionEnabled`; normalizes `oauth`), so an entry authored outside the app is loaded and then re-serialized, not kept byte-for-byte. Still unverified: that Cowork lists a copied skill without an app-written manifest `updatedAt` (look at the Cowork skills list on this Mac); that a boot-slapper-owned v2 entry survives the same rewrite (no v2 entry has been applied on a real box yet); that Claude Desktop runs a `.ps1` helper through `powershell.exe -File` (taken from the bundle's interpreter table, not observed — spikes S1/S2 could not extract the spawn code); if the Windows box disproves it, the owner types the key into the in-app window and a static-credential profile option becomes a follow-up.
