...
| 9 | `desktop-mcp` → 8 | desktop | translatable | write MeCP + Open Brain into `claude_desktop_config.json`; HTTP-with-header if supported, else `mcp-remote` stdio bridge — adapter chosen by a runtime capability probe and reported by `verify` | new — **amended 2026-09-24:** the bundle-derived list gained `cornell_secure_tools`, authenticated with `x-litellm-api-key` through a headers helper (any bundle header with a mapped `${VAR}` placeholder, not only `Authorization`); the gateway's `/mcp/` accepts a direct streamable-HTTP client with that header, so KB 9077's `mcp-remote` bridge is unnecessary on either surface |
