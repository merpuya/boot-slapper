...
8. **`requires` edges beyond the spec's `8 → 3`, `9 → 8`, `10 → 2`.** `desktop-inference → prereqs`, `desktop-mcp → claude-config` (it reads the tracked `gateway.json`), `desktop-skills → desktop-inference` (it needs the applied entry's `deploymentOrganizationUuid`); additive, DAG-sound, order unchanged.
9. **The `.ps1` helper mechanism (`powershell.exe -File`) is a gate-2 assumption from the bundle's interpreter table.** The static `apiKey`/`headers` fallback the spikes suggested is deliberately not written (no boot-slapper-written secret value); the kept degraded path is manual entry in the in-app window.
